from django.urls import path
from .views import (
    results_view,
    ListingDetailView,
    AllProductsView,
    AllProductsPriceHighView,
    AllProductsPriceLowView,
    FamousView,
    FamousPriceHighView,
    FamousPriceLowView,
    SpecialView,
    SpecialPriceHighView,
    SpecialPriceLowView,
    HarryView,
    HarryPriceHighView,
    HarryPriceLowView,
    starwars_category_view,
    misc_category_view,
    jobs_category_view,
    doctor_who_category_view,
    xmas_category_view,
    halloween_category_view,
    categories_view
    )

urlpatterns = [
    path('listing/<int:pk>/', ListingDetailView.as_view(), name="listing-detail"),
    path('categories/', categories_view, name="categories"),
    path('results/', results_view, name="results"),
    path('all-products/', AllProductsView.as_view(), name="all-products"),
    path('all-products/price-high/', AllProductsPriceHighView.as_view(), name="all-products-price-high"),
    path('all-products/price-low/', AllProductsPriceLowView.as_view(), name="all-products-price-low"),
    path('categories/famous', FamousView.as_view(), name="famous"),
    path('categories/famous/price-high/', FamousPriceHighView.as_view(), name="famous-price-high"),
    path('categories/famous/price-low/', FamousPriceLowView.as_view(), name="famous-price-low"),
    path('categories/special-occasions', SpecialView.as_view(), name="special-occasions"),
    path('categories/special-occasions/price-high/', SpecialPriceHighView.as_view(), name="special-occasions-price-high"),
    path('categories/special-occasions/price-low/', SpecialPriceLowView.as_view(), name="special-occasions-price-low"),
    path('categories/harry-potter', HarryView.as_view(), name="harry-potter"),
    path('categories/harry-potter/price-high/', HarryView.as_view(), name="harry-potter-price-high"),
    path('categories/harry-potter/price-low/', HarryView.as_view(), name="harry-potter-price-low"),
    path('categories/star-wars', starwars_category_view, name="star-wars"),
    path('categories/weird-misc', misc_category_view, name="weird-misc"),
    path('categories/jobs-hobbies', jobs_category_view, name="jobs-hobbies"),
    path('categories/doctor-who', doctor_who_category_view, name="doctor-who"),
    path('categories/christmas', xmas_category_view, name="christmas"),
    path('categories/halloween', halloween_category_view, name="halloween"),
]